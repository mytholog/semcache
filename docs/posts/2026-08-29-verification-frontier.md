...
- **On v1 the gate's recall cost is unmeasurable.** Every interchangeable pair is English, so "0 lost" only says the gate does not reject monolingual English paraphrases. v2 (`bench/dataset/v2.jsonl`) adds an answer and `answer_lang` on every pair, plus same-language paraphrase and format pairs for each language in the switch set. Against the recorded language the gate keeps 229/229 positives inside the detector and catches 60/65 language switches. Chinese pairs are in the file and invisible to the detector. The new positives are templates, not a reviewed trace.
