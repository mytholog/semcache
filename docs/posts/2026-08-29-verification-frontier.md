...
- 4.9% false-hit is 8× better than a threshold and still far too high for a billing or auth answer. The category table says where the remaining risk is, which is the point of publishing it.

## Next

M3: eager tagged invalidation in Postgres, atomic with vector removal. The `language_switch` leak is a verifier fix and does not wait for M3.
